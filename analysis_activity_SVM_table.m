...
for repID = [2:6]
    ...
    for eventID = 1:length(Table{repID}.stepStartIdxArray)
        ...
        if Table{repID}.label(eventID) == 7
            continue;
        end
        ...
        FeatureSet{repID}.Label = [FeatureSet{repID}.Label; Table{repID}.label(eventID)];      
    end
end
...
save('result_SVM_Table.mat','Results');
save('feature_set_table.mat','FeatureSet');
...
totalAcc2 = sum(totalTest2 == totalLabel)/length(totalLabel)
