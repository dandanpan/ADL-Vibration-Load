function [ stepEventsIdx, stepEventsVal, ...
            ...
            windowEnergyArray, noiseMu, noiseSigma, noiseRange ] = SEDetectionFlex( rawSig, noiseSig, windowSize, sigmaSize )
...
%     sigmaSize = 16;
    ...
    while idx < length(signal) - max(windowSize, eventSize)*2
%         idx
    ...
    if states == 1
        ...
        stepStopIdx = min(stepStartIdx + eventSize - 1, length(rawSig));
        stepSig = rawSig(stepStartIdx:stepStopIdx);
        stepStartIdxArray = [stepStartIdxArray, stepStartIdx];
        stepStopIdxArray = [stepStopIdxArray, stepStopIdx];
