...
finalResults = [];
for windowID = 1:5
    load(['eval_current_window' num2str(windowID) '.mat']);
    Result_Current = Results(2:6);
    load(['eval_current_4act_window' num2str(windowID) '.mat']);
    Result_Current = [Result_Current, Results];
    
    load(['eval_table_window' num2str(windowID) '.mat']);
    Result_Table = Results(2:6);
    load(['eval_table_4act_window' num2str(windowID) '.mat']);
    Result_Table = [Result_Table, Results];

    load(['eval_floor_window' num2str(windowID) '.mat']);
    Result_Floor = Results(2:6);
    load(['eval_floor_4act_window' num2str(windowID) '.mat']);
...
%         plot(estimation2{repID}); ylim([0,15]);
    ...
    finalResults = [finalResults; meanTable2, meanFloor2, meanCurrent2, meanFuse_cf, meanFuse2];
...
legend({'Fs/32','Fs/16','Fs/8','Fs/4','Fs/2'});
xlim([0.5,5.5]);
ylim([0,1]);