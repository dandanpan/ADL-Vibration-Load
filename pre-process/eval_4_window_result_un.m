...
for sensorID = 1:3
    ...
    for setID = 1:totalSetNum
        if setID <= 5
            ...
            clear data;
        elseif setID <= 9
            load(['/Users/shijiapan/Box Sync/Activity_Detection/0408/PortLab_20190408_Route_P3_Rep' num2str(repID{2}(setID-5)) '.mat']);
            dataLens{setID} = length(data);
            clear data;
        else
            load(['/Users/shijiapan/Box Sync/Activity_Detection/0429/PortLab_20190429_Route_P4_Rep' num2str(repID{3}(setID-9)) '.mat']);
            dataLens{setID} = length(data);
            clear data;
        end
    end
    ...
    for setID = 1:totalSetNum
        ...
        elseif setID <= 9
            totalEventStart = [totalEventStart Sensor{2,sensorID}{repID{2}(setID-5)}.stepStartIdxArray+offset]; 
            totalEventStop = [totalEventStop Sensor{2,sensorID}{repID{2}(setID-5)}.stepStopIdxArray+offset]; 
        else
            totalEventStart = [totalEventStart Sensor{3,sensorID}{repID{3}(setID-9)}.stepStartIdxArray+offset]; 
            totalEventStop = [totalEventStop Sensor{3,sensorID}{repID{3}(setID-9)}.stepStopIdxArray+offset]; 
        end
...
save('eval_p_un.mat','SensorResults');
return;
