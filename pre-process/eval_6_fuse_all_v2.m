...
finalResult = [tableAvg; floorAvg; currentAvg; v2Avg; ourAvg]';
...
mean(ourAvg)
finalResult_same = finalResult_new;
save('finalResults.mat','finalResult_same');
...
totalEventNum = 0;
for repID = 1:5
    gt_stat_rep = gt1{repID}; % SensorResults{1, repID}.ObservLabelTest;
...
plotOrder = [1,2,3,8,4,5,6,7];
event_stat_duration_new = event_stat_duration;
event_stat_number_new = event_stat_number;
for i = 1:length(plotOrder)
    event_stat_duration_new(i) = event_stat_duration(plotOrder(i));
    event_stat_number_new(i) = event_stat_number(plotOrder(i));
end

figure;
subplot(2,1,1);
bar((event_stat_duration_new./2+1)./8);
xticklabels({'K', 'OM','M','MD','PS','S','V','Step'});
ylabel('Event Duration (s)');
subplot(2,1,2);
bar(event_stat_number_new);
xticklabels({'K', 'OM','M','MD','PS','S','V','Step'});
ylabel('Event Number');
